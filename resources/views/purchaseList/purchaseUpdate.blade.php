...
<div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          ...
          <form action="{{url('purchUpdate')}}" method="post" class="was-validated" enctype="multipart/form-data"> 
          @csrf  
          <div class="col-sm-12">
            <div class="row">
           <div class="col-sm-6" >
           ...
           <div class="col-sm-9" >
            <b>Supplier Name</b> 
            <input type="text" class="form-control" name="Supplier_Name" value="{{$data->Sup_Name}}" style="background-color:#ffcccb" required> 
          </div>
         
          <div class="col-sm-9" >
           <b>Purchase Proof</b> 
           <input type="file" class="form-control" name="file" style="padding-top: 3px" value="{{$data->Pur_Proof}}"  required> 
         </div>
           ...
           <div class="col-sm-9" style="margin-left: 140px" >
            <b>Purchase Date</b> 
            <input type="text" class="form-control" name="Supplier_Date" id="datepicker" value="{{$data->Sup_Date}}" required> 
          </div> 
          
          <div class="col-sm-9" style="margin-left: 140px" >
           <b>Purchase Description</b> 
           <input type="text" class="form-control" name="Description" value="{{$data->Desc}}" required> 
         </div>
         
          <div class="col-sm-9"  style="margin-left: 140px" >
           <b>Total Amount</b> 
           
             <input type="text" name="total" class="form-control" id="totall" value="{{$data->Total}}" readonly>
         ...
         <td>
          <select name="Stock_Item[]"  class="custom-select item" style="font-size: 15px;" required>
            ...
            <option  name="Stock_Item[]" value="{{$phar->Item_Name}}">{{$phar->Item_Name}}</option>
              @endforeach
            </select>
        </td>
         <td><input type="text" name="Batch[]" class="form-control" required></td>  
           <td><input type="text" name="Count[]" class="form-control count" readonly></td>
           <td><input type="text" name="Cost[]" class="form-control cost" readonly></td>
...
<script type="text/javascript">
   $('tbody').on('change','.item',function(){
    ...
    {
        var tr='<tr>'+
        '<td><select name="Stock_Item[]"  class="custom-select item" style="font-size: 15px;" required> <option name="Stock_Item[]" value=""></option>   @foreach(App\ pharmacy::all() as $phar)   <option  name="Stock_Item[]" value="{{$phar->Item_Name}}">{{$phar->Item_Name}}</option> @endforeach</select></td>'+
        '<td><input type="text" name="Batch[]" class="form-control"></td>'+
        '<td><input type="text" name="Count[]" class="form-control count" required=""></td>'+
        '<td><input type="text" name="Cost[]" class="form-control cost"></td>'+
        ' <td><input type="text" name="Total[]" class="form-control amont"></td>'+
        '<td><a href="#" class="btn btn-danger RemoveRow"><i class="fas fa-trash-alt"></i></a></td>'+
        '</tr>';
        $('tbody').append(tr);
    };
